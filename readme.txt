...
* Fix - Changed <code>'p'</code> argument name to <code>'psp'</code> argument name inside legacy api url to fix conflict with WordPress 'p' Post ID argument
...
= 2.0.4 - 2013/06/12 =
* Tweak - Search box dropdown now can show the full wide when search function is activated by a third party script. An example of this is when the search box is not loaded on page load but is activated by click on a button or element.
...
* Tweak - Added when install and activate plugin link redirects to the Predictive Search admin page instead of the wp-plugins dashboard.
